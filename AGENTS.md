...
- 虚拟按键常驻：左下摇杆 `MoveStick`，右下大圆攻击（无字）+ 跳/冲刺/交谈圆钮。键盘 WASD/J/K/空格/E 仍可用。摇杆优先当瞄准方向。
- 镜头跟随英雄。地板向北扩出商店翼 `SHOP_WING`（y=-400..112）。战斗房在左（核心），东侧开阔地砖房，路口向外铺 `ROAD_EAST` / `ROAD_NORTH` / `ROAD_SOUTH`。上下敌人出口是抠好的漩涡门动画 `SpawnPortalNorth` / `SpawnPortalSouth`（路尽头）。原房南墙仍在 y=640。北墙一道南门进商人 / 训练师大厅。核心西侧仍可走 `HOME_HALL`（x=-80..76）。镜头钳在 `FLOOR_BOUNDS`。
- 建造容量 = 8 座。战斗房地砖格对齐 atlas 64 缝（`GRID_OX=0`，`GRID_OY=-8`）。点地砖放下，点已放塔选中。核心台 `CORE_PLATFORM` 整块不能放。商店武器买入后点地砖放下成自动炮（catalog 贴图 + 弹道，不算新塔 ID），仍占 8 座；英雄双槽仍走拾取/装备。塔只绕行不封路。
