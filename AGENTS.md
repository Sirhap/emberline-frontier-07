...
- 入口是 `scenes/app_root.tscn`（家园 `HomeHub` + `HomeRoom`）。点「开始远征」开新局（档案上次英雄，缺省骑士）。继续远征用存档里的英雄。家园不摆选人石座、不摆传送门。大厅画面用 `assets/generated/home/layout-ref.png` 烘焙成 `floor-room.png`，不再另贴家具。点书架 / BUG 灯牌 / 白板 / 懒人沙发浏览图鉴或提示宠物未开放。烟测仍直接加载 `main.tscn`，默认骑士局。
