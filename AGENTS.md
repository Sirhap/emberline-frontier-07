...
- `WaveDirector`：`PREP` 50s → `COMBAT` → 配额清完且场上无活怪（1.0s 防抖）→ `scrap += 50`，并在核心一侧 `HOME_REWARD_SPOTS` 刷 3 个传送带奖励（废料锭 / 药剂 / 武器）→ 再准备。地上奖励不走近吸拾：点选或「拾取」收一件，「丢弃」或 20s 超时进仓库。失败条件：核心 0 **或** 英雄倒地。上房间、战场、下房间是**三间分开的地牢房**（墙不贴墙）：北 **上房间**机械师+商人，中间 **COMBAT_ROOM** 核心+传送带，南 **下房间**召唤师+军官+导师。房间之间是门洞+走廊。准备期 `SHOP_SHELVES` 为台座+浮空图标，点台座可买。刷怪 `4 + wave * 2`（潮汐波 ×1.55）；间隔 `max(0.40, 0.90 - wave * 0.04)`。
...
- 镜头跟随英雄。上房间与战场北墙之间、下房间与战场南墙之间都是走廊，三间房不挨着。墙是金边石墙，门是墙上开的洞，地砖从门洞连进去。东扩与南北口按格铺走廊，口是金边石墙上的洞；传送门嵌在洞里（洞内是黑洞，不是地砖），没激活时是封石。最多 6 门（每 90 波多开 1 门），红=激活 / 蓝=闲置。`SpawnPortalNorth` / `NorthW` / `NorthE` / `SpawnPortalSouth` / `SouthW` / `SpawnPortalEast`。原房南墙仍在 y=640。核心西侧仍可走 `HOME_HALL`。镜头钳在 `FLOOR_BOUNDS`。
