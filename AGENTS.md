...
- 入口是 `scenes/app_root.tscn`（家园 `HomeHub` + `HomeRoom`）。点「开始远征」开新局（档案上次英雄，缺省骑士）。继续远征用存档里的英雄。家园不摆选人石座、不摆传送门。家园地面/家具只从 `assets/generated/home/` 拼接，不引用战场地板。点兵器图鉴 / 敌人图鉴 / 战绩碑浏览 `user://meta.json`（未发现为 `???`）；宠物巢穴只提示未开放。烟测仍直接加载 `main.tscn`，默认骑士局。
