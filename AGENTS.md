...
- `WaveDirector`：`PREP` 50s → `COMBAT` → 配额清完且场上无活怪（1.0s 防抖）→ `scrap += 50`，并在核心一侧 `HOME_REWARD_SPOTS` 刷 3 个传送带奖励（废料锭 / 药剂 / 武器）→ 再准备。地上奖励不走近吸拾：点选或「拾取」收一件，「丢弃」或 20s 超时进仓库。失败条件：核心 0 **或** 英雄倒地。北侧准备区是两间 SK 式地牢房（不对标乱加通栏黄杠）：北 **商人厅**（机械师+商人货摊），南 **导师厅**（召唤师+军官+导师），中间门洞连通，导师厅南门进战场。准备期 `SHOP_SHELVES` 为台座+浮空图标，点台座可买。刷怪 `4 + wave * 2`（潮汐波 ×1.55）；间隔 `max(0.40, 0.90 - wave * 0.04)`。
...
- 准备期走近柜台（约 110px）点货买入。武器进双槽，炮台进英雄仓库（同种叠数）。炮台手点空地砖放下，准备和作战都行。武器手点空炮台把当前武器装上；点已有武器的炮台则交换。商人厅：机械师站修塔台座旁、商人站三座炮台旁；导师厅：召唤师站召唤台座旁、军官只交谈、导师站锻造/技能/成长台座旁。买完对应 NPC 跑到该台座播补货动画后回站位（第 1 波 pulse / burst / frost）。开战不退武器槽。空建造垫是灰金属台+绿箭头；装枪后仍占座。
