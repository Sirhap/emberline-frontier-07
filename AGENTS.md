...
| 英雄 | 点无尽之门进塔防，默认骑士 `ember_hero`；档案 `last_selected_hero` 为刺客时用 `assassin`。局内 HUD 头像只展示，不切人。骑士/刺客技能仍走冲刺槽：骑士浮剑 `1/2/3` 发，刺客 `3 + skill_level` 影分身（最多 6）。局内 Lv1–10 + 三选一天赋。 |
...
- 入口是 `scenes/app_root.tscn`（家园 `HomeHub` + `HomeRoom`）。点无尽之门开新局（档案上次英雄，缺省骑士）。继续远征用存档里的英雄。家园不摆选人石座。家园地面/家具只从 `assets/generated/home/` 拼接，不引用战场地板。点兵器图鉴 / 敌人图鉴 / 战绩碑浏览 `user://meta.json`（未发现为 `???`）；宠物巢穴只提示未开放。烟测仍直接加载 `main.tscn`，默认骑士局。
