...
- `WaveDirector`：`PREP` 50s → `COMBAT` → 配额清完且场上无活怪（1.0s 防抖）→ `scrap += 50`，并在核心一侧 `HOME_REWARD_SPOTS` 刷 3 个传送带奖励（废料锭 / 药剂 / 武器）→ 再准备。地上奖励不走近吸拾：点选或「拾取」收一件，「丢弃」或 20s 超时进仓库。失败条件：核心 0 **或** 英雄倒地。战场按元气骑士图砌成三个横向房间：**上房间**机械师+商人货摊，**中间战斗过道**核心+传送带，**下房间**召唤师+军官+导师。金砖矮墙是房间隔墙（不是地上黄线），约 2/3 处停，**东突破**上下走。准备期 `SHOP_SHELVES` 为台座+浮空图标，点台座可买。刷怪 `4 + wave * 2`（潮汐波 ×1.55）；间隔 `max(0.40, 0.90 - wave * 0.04)`。
- 开局 300 废料、核心 10、英雄剑、100 血、冲刺/影分身已解锁。导师：锻造、技能、导师成长（生命/能量/护盾循环）。机械师：机械修复（全场修塔）。召唤师：随机废料/回血/金矿/炸伤；波≥3 可买半价天赋。倒地结束本局。右下可选骑士 / 刺客。
...
- 镜头跟随英雄。上房间 / 中间战斗过道 / 下房间是同一套地砖砌的三个横向房间，金砖墙隔开，东突破连通。不要再在战场北侧另叠准备厅。东扩与南北口按格铺走廊，口是金边石墙上的洞；传送门嵌在洞里（洞内是黑洞，不是地砖），没激活时是封石。最多 6 门（每 90 波多开 1 门），红=激活 / 蓝=闲置。`SpawnPortalNorth` / `NorthW` / `NorthE` / `SpawnPortalSouth` / `SouthW` / `SpawnPortalEast`。原房南墙仍在 y=640。核心西侧仍可走 `HOME_HALL`。镜头钳在 `FLOOR_BOUNDS`。
...
- 准备期走近柜台（约 110px）点货买入。武器进双槽，炮台进英雄仓库（同种叠数）。炮台手点空地砖放下，准备和作战都行。武器手点空炮台把当前武器装上；点已有武器的炮台则交换。上厅：机械师站修塔台座旁、商人站三座炮台旁；下厅：召唤师站召唤台座旁、军官只交谈、导师站锻造/技能/成长台座旁。中间过道不摆货摊。买完对应 NPC 跑到该台座播补货动画后回站位（第 1 波 pulse / burst / frost）。开战不退武器槽。空建造垫是灰金属台+绿箭头；装枪后仍占座。
