...
- 入口是 `scenes/app_root.tscn`（家园 `HomeHub` + `HomeRoom`）。点「开始远征」开新局（档案上次英雄，缺省骑士）。继续远征用存档里的英雄。家园不摆选人石座、不摆传送门。空房间底图烘焙成 `floor-room.png`，家具从 `station-pack` / `tech-pack` 黑底抠出后按参考图位置摆上；禁止把整张 `layout-ref` 当地板，也不从参考图抠带地砖的矩形。点书架 / 展示柜 / 石碑 / 沙发浏览图鉴或提示宠物未开放。烟测仍直接加载 `main.tscn`，默认骑士局。
