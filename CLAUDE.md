...
| NPC | 商人 `merchant`、训练师 `trainer`。点柜台购买，`E` 交谈可选 |
...
- 开局 300 废料、核心 10、英雄剑、100 血、冲刺/影分身已解锁。训练师两格常驻：锻造（当前武器 +10% 攻击/级，最多 5）和技能提升（柜上不写具体技能名）。倒地 4s，在核心附近以 40 血复活。局不结束。右下可选骑士 / 刺客。
...
- 准备期走近柜台（约 110px）点货买入。武器进双槽，炮台进英雄仓库（同种叠数）。炮台手点空地砖放下，准备和作战都行。武器手点空炮台把当前武器装上；点已有武器的炮台则交换。空炮台仍打 pulse/burst/frost。商人 3 柜台只卖炮台，买完立刻补货（第 1 波 pulse / burst / frost）。开战不退武器槽。
