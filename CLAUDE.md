...
| 英雄 | 开局骑士 `ember_hero`；刺客 `assassin`。右下圆钮切换。刺客技能走现有冲刺槽：转圈绿阵同时刷多重冒泡影分身 |
...
- 开局 300 废料、核心 10、英雄剑、100 血、无冲刺。倒地 4s，在核心附近以 40 血复活。局不结束。右下可选骑士 / 刺客。
- 双武器槽（元气骑士）：右下攻击键左侧一个切换圆钮，显示当前武器，点钮或 `Q` 在两槽间切换。新枪填空槽，两槽都满则替换当前槽。
- 虚拟按键常驻：左下摇杆 `MoveStick`，右下大圆攻击（无字）+ 跳/冲刺或影分身/交谈圆钮。键盘 WASD/J/K/空格/E 仍可用。摇杆优先当瞄准方向。刺客技能仍走训练师解锁的冲刺槽：释放转圈绿阵时身边刷多重冒泡影分身。
...
- 建造容量 = 8 座。战斗房地砖格对齐 atlas 64 缝。墙/口仍用 `GRID_OX=0`，`GRID_OY=-8`（背景图顶边）；放置预览和铺砖用 `FLOOR_GRID_OX` / `FLOOR_GRID_OY`（图里地砖缝相位 atlas 4, 42）。点地砖放下，点已放塔选中。核心台 `CORE_PLATFORM` 整块不能放。商店武器买入后点地砖放下成自动炮（catalog 贴图 + 弹道，不算新塔 ID），仍占 8 座；英雄双槽仍走拾取/装备。塔只绕行不封路。
- 准备期必须走近商人按 `E` 买入后再点地放下；未持塔点地不放。作战可在空地砖上建造（未缩放 `build_cost`）和升级。开战 `close_and_refund()` 退未放置持塔。
...
开着时额外画出：敌人 96/144 圈、HUD `E n/40  B n/120  T n/8`。面板在 `(16, 280)`，避开摇杆和右侧塔信息。
