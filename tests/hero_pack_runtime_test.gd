...
func _run() -> void:
	...
	assert(bool(hero.get("_transforming")), "clip may continue after input unlock")
	assert(hero.visual_pack_id == &"frost_warrior", "pack swaps only when cast clip ends")
	assert(not bool(hero.call("_skill_controls_locked")), "controls unlock at 1.2s cap")
	hero.call("_update_dash", maxf(cast_clip - EmberHero.SKILL_INPUT_LOCK_CAP, 0.0) + 0.05)
	assert(hero.visual_pack_id == &"frost_armed", "transform commits when cast clip ends")
	assert(float(hero.get("_dash_elapsed")) < 0.0, "dash clears when cast ends")
