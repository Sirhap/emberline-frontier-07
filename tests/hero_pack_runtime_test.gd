...
func _run() -> void:
	...
	assert(hero.visual_pack_id == &"frost_armed", "transform swaps by input-lock cap, not full clip")
	assert(float(hero.get("_dash_elapsed")) < 0.0, "dash input lock cleared at cap")
	...
	hero.call("_update_dash", EmberHero.SKILL_INPUT_LOCK_CAP + 0.05)
	assert(hero.visual_pack_id == &"frost_warrior", "untransform returns by input-lock cap")
