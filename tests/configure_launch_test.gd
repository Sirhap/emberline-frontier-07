...
func _init() -> void:
	# ignore_pause: _end_run / notify_hero_defeated can leave the tree paused.
	create_timer(40.0, true, true, true).timeout.connect(func() -> void:
		push_error("configure_launch_test watchdog")
		quit(1)
	)
	call_deferred("_run")
