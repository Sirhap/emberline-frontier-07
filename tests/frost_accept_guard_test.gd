...
## Skill commit (T1.2) keeps the hero and core alive for 3s, then both can fall.
func _run() -> void:
	...
	assert(not bool(hero.call("is_frost_accept_guarded")), "guard expires after 3s even if still armed")
	assert(hero.form_left > 0.0, "armed form is still running when the shield drops")
	leaker.call("_follow_seek", 40.0)
	assert(int(scene.get("core_health")) < core_before, "core can fall after the 3s accept guard")
	scene.queue_free()
	await process_frame
