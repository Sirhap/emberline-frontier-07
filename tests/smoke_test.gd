...
func _run_smoke_test() -> void:
	...
	assert(load("res://assets/generated/towers/burst-lv1.png") != null, "Legacy burst art still imports")
	...
	assert(is_equal_approx(float(switched.call("combat_visual_scale")), EmberHero.COMBAT_VISUAL_SCALE), "Knight skill 0 stays at combat scale")
	...
	assert(float(switched.call("skill_size_mult")) > 1.0, "Knight skill 1 grows range/damage mult")
	assert(is_equal_approx(float(switched.call("combat_visual_scale")), EmberHero.COMBAT_VISUAL_SCALE), "Knight skill 1 does not scale the sprite")
	...
	scene.set("scrap", 500)
	scene.call("_spawn_tower_at", Vector2(820.0, 280.0), &"hologram", 1)
	scene.call("_spawn_tower_at", Vector2(650.0, 280.0), &"frost", 1)
	var towers: Array = scene.get("_towers")
	assert(towers.size() == 3, "Pulse, hologram, and frost towers should all place")
	assert(towers[1].kind == &"hologram", "Second pad should hold a v1 hologram pad")
	assert(towers[2].kind == &"frost", "Third pad should hold a frost tower")
	assert(towers[0].kind != &"burst" and towers[1].kind != &"burst" and towers[2].kind != &"burst", "Formal place path is v1 kinds, not leftover burst")
	scene.set("scrap", 800)
	...
	assert(float(hero.call("skill_size_mult")) > 1.0, "Live N x2 should grow knight range/damage")
	assert(is_equal_approx(float(hero.call("combat_visual_scale")), EmberHero.COMBAT_VISUAL_SCALE), "Live N x2 does not scale the sprite")
