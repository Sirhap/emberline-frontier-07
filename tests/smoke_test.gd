...
func _run_smoke_test() -> void:
	...
	assert(mech_npc.position.y < (shelves[0] as Vector2).y, "Top NPCs stand behind counters, facing the south door")
	assert(sum_npc.position.y > (shelves[4] as Vector2).y, "Bottom NPCs stand behind counters, facing the north door")
