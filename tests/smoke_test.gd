...
func _run_smoke_test() -> void:
	...
	assert(trn_rest.y <= -40.0 and trn_rest.y >= -120.0, "Mentor rest stand is on the bottom stall row")
	var crate_block: Vector2 = hero.position
	hero.position = Vector2(420.0, -220.0)
	var blocked: Vector2 = scene.call("_clamp_to_walkable", Vector2(420.0, -140.0), Vector2(420.0, -220.0))
	assert(blocked.y > -200.0, "Hero should not walk onto a shop pedestal")
	...
	assert(mer_room == trn_room, "One prep hall — not two stacked rooms")
	...
	scene.call("_try_buy_shelf", Vector2(510.0, -220.0))
	...
	scene.call("_try_buy_shelf", Vector2(420.0, -220.0))
	...
	hero.position = Vector2(560.0, -20.0)
	scene.call("_sync_skill_hud")
