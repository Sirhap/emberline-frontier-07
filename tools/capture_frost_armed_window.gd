...
func _run() -> void:
	...
	assert(jump_marks.visible and (jump_marks.get_node("JumpColL") as CanvasItem).visible, "JUMP dump must show the air-gap columns")
	assert(not (jump_marks.get_node("SlashMid") as CanvasItem).visible, "JUMP dump must not reuse the slash overlay")
	await _wait(0.70)
	...
	assert(atk_marks.visible and (atk_marks.get_node("SlashMid") as CanvasItem).visible, "ATTACK dump must show the ice crescent")
	assert(not (atk_marks.get_node("JumpColL") as CanvasItem).visible, "ATTACK dump must not reuse the jump overlay")
	await _wait(0.70)
