...
5. **必须在 T1.2 之后 3 秒内截到三帧（缺一不可）：**
   ...
   - `LIVE-JUMP`：腾空跳帧，脚离地 / 收腿，不是贴地站桩。K / 跳钮。持械跳立刻抬约 128px、锁收腿，脚下留大阴影和两根冰柱。护盾还在时会一直钉到窗口结束（至少 1.1s）。JUMP 和 ATTACK 不能是同一张站桩。
   - `LIVE-ATTACK`：明确横向冰弧（角色前方三道冰刃），HP>0、核心未掉。J / 攻击钮。持械 J 立刻打断跳跃并锁侧向平挥，冰弧画在角色前面。竖直橙剑 / 待机持剑 / 倒地复活不算。
6. 倒地复活条或「核心失守」结算 = 失败证据。HUD「停」或 Esc / P 暂停，点「设」。

通过条件：选角能进；T1.2 起 3 秒内持械三帧都清楚且未倒地/核爆；暂停「设」可点。不要用 Chrome LOAD_BLOCKED 当桌面包失败。

merge 后重测：`./tools/export_linux_accept.sh` 打新包，验收机 `DISPLAY=:9` 跑新 `linux-accept-<shortsha>.tar.gz`，不要用 tip `867ad3e` / `3d2ca71` 的旧包。
