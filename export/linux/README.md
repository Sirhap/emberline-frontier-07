...
4. **T1.2 起持械窗口（约 8s）有玩法护盾**（无敌 + 漏怪/刷怪暂缓）。至少前 3 秒英雄必须活着、核心必须还在；解除持械后恢复正常。
5. **持械窗口内必须截到三帧（缺一不可，建议 T1.2 后 3 秒内拍完）：**
...
通过条件：选角能进；持械窗口内三帧都清楚且未倒地/核爆；暂停「设」可点。不要用 Chrome LOAD_BLOCKED 当桌面包失败。
